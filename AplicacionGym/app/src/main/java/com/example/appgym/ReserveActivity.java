...
public class ReserveActivity extends AppCompatActivity {
    ...
    private List<Activities> reserveActivities;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reservationRecyclerView = findViewById(R.id.reservation_list);
        ...
        ActionBarDrawerToggle actionBarDrawerToggle= new ActionBarDrawerToggle(ReserveActivity.this,
        ...
        Reserve.setAllActivities(myActivities);

        reservationRecyclerView = findViewById(R.id.reservation_list);
        reserveActivities = Reserve.getReservations(this);

        if (reserveActivities != null) {
            reserveAdapter = new ReserveAdapter(this, reserveActivities);
            reservationRecyclerView.setLayoutManager(new LinearLayoutManager(this));
            reservationRecyclerView.setAdapter(reserveAdapter);

            // Obtén las actividades favoritas y actualiza el adaptador
            updateReserveActivitiesList();
        } else {
    ...
    private void updateReserveActivitiesList() {
        reserveActivities = Reserve.getReservations(this);

        // Agrega log para verificar la lista de favoritos
        for (Activities activity : reserveActivities) {
            Log.d("ReserveActivity", "Reserve activity: " + activity.getActivitytitle());
        }
        reserveAdapter.setActivitiesList(reserveActivities);
        reserveAdapter.notifyDataSetChanged();

        if (reserveActivities.isEmpty()) {
            showToast("No hay actividades Reservadas");
        }
    }
